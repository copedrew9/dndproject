...
def typed_q(prompt, tail):
    """q at the third prompt that offers it, which is well inside the
    wizard rather than on its first screen."""
...
def settings_survive(binary, workdir, seed):
    """Toggles a rule in the wizard's own step 0, types q, and reads the
    main menu's settings screen back.

    Step 0 settles which books this character draws on. The restart path has
    always put them back; leaving has to as well, or a character thrown away
    takes the rest of the session's content settings with it."""
    proc = subprocess.Popen([binary, "--seed", str(seed)],
                            stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE, bufsize=0, cwd=workdir)
    transcript = []
    state = ["create", "toggle", "quit", "settings", "read"]
    at = 0
    seen = None
    ...
    try:
        while at < len(state):
            ...
            if step == "create" and MAIN_MENU in tail:
                reply = "1"
            elif step == "toggle" and "Optional rules" in tail:
                reply = drive.menu_number(tail, ROW)
                if not reply:
                    raise SystemExit("verify_quit: no optional-features row")
            elif step == "quit" and "Optional rules" in tail:
                ...
                reply = "q"
            elif step == "settings" and MAIN_MENU in tail:
                reply = drive.menu_number(tail, "Content settings")
            elif step == "read" and "Optional rules" in tail:
                seen = tail
                break
            else:
                continue        # a screen between the steps; read on
            at += 1
            proc.stdin.write((reply + "\n").encode())
            proc.stdin.flush()
    except BrokenPipeError:
    ...
    if last_row(seen) != "[x]":
        raise SystemExit("a thrown-away character took the session's content "
                         "settings with it:\n%s" % seen[-800:])
...
def main():
    ...
    with tempfile.TemporaryDirectory() as workdir:
        ...
        settings_survive(binary, workdir, 13)
